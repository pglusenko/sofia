...
// Малювання ліній графічного ключа на Canvas
...
// =================================================================
// 2. ДИНАМІЧНЕ РОЗШИФРУВАННЯ ТА СКЛІЙКА ВВЕДЕНОГО КЛЮЧА
// =================================================================
function handleEnd() {
    if (!isTracking) return;
    isTracking = false;
    
    // Перетворюємо пройдений шлях у текстовий цифровий ключ
    const userPattern = activeNodes.map(n => n.getAttribute('data-index')).join('');
    
    // Робимо спробу декодувати першу фотку цим вводом
    const url1 = decrypt(ENCRYPTED_PHOTOS[0], userPattern);
    
    // Перевірка на валідність результату (чи розшифрувався лінк)
    if (url1 && url1.startsWith('http')) {
        // Якщо посилання правильне — розшифровуємо і розставляємо всі 9 фотографій
        ...
        // Прибираємо локскрін і пускаємо на сайт
        ...
        // Помилка: підсвічуємо лінії червоним і скидаємо
